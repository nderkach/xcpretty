...
module XCPretty
  ...
  describe Parser do
    ...
    it "parses analyze" do
      expect(@formatter).to receive(:format_analyze).with("CCChip8DisplayView.m", "CocoaChip/CCChip8DisplayView.m", "foo")
    ...
    it "parses analyze for a C++ target" do
      expect(@formatter).to receive(:format_analyze).with("CCChip8DisplayView.cpp", "CocoaChip/CCChip8DisplayView.cpp", "foo")
      @parser.parse(SAMPLE_ANALYZE_CPP)
    end

    it "parses build target" do
      expect(@formatter).to receive(:format_build_target).with("The Spacer", "Pods", "Debug")
      @parser.parse(SAMPLE_BUILD)
    end

    it "parses aggregate target" do
      expect(@formatter).to receive(:format_aggregate_target).with("Be Aggro", "AggregateExample", "Debug")
      @parser.parse(SAMPLE_AGGREGATE_TARGET)
    end

    it "parses analyze target" do
      expect(@formatter).to receive(:format_analyze_target).with("The Spacer", "Pods", "Debug")
      @parser.parse(SAMPLE_ANALYZE_TARGET)
    end

    it "parses clean remove" do
      expect(@formatter).to receive(:format_clean_remove)
      @parser.parse(SAMPLE_CLEAN_REMOVE)
    end

    it "parses clean target" do
      expect(@formatter).to receive(:format_clean_target).with("Pods-ObjectiveSugar", "Pods", "Debug")
      @parser.parse(SAMPLE_CLEAN)
    end

    it "parses clean target withut dash in target name" do
      expect(@formatter).to receive(:format_clean_target).with("Pods", "Pods", "Debug")
      @parser.parse(SAMPLE_ANOTHER_CLEAN)
    end

    it "parses check dependencies" do
      expect(@formatter).to receive(:format_check_dependencies)
      @parser.parse("Check dependencies")
    end

    it "parses code signing" do
      expect(@formatter).to receive(:format_codesign).with("build/Release/CocoaChip.app", "foo")
      @parser.parse(SAMPLE_CODESIGN)
    end

    it "parses code signing a framework" do
      expect(@formatter).to receive(:format_codesign).with("build/Release/CocoaChipCore.framework", "foo")
      @parser.parse(SAMPLE_CODESIGN_FRAMEWORK)
    end

    it 'parses compiler_space_in_path' do
      expect(@formatter).to receive(:format_compile).with('SASellableItem.m', "SACore/App/Models/Core\\ Data/human/SASellableItem.m", "foo")
    ...
    it "parses compiler commands" do
      ...
      expect(@formatter).to receive(:format_compile_command).with(compile_statement.strip, "/Users/musalj/code/OSS/Kiwi/Classes/Core/KWNull.m", nil)
      @parser.parse(compile_statement)
    end

    it "parses compiling categories" do
      expect(@formatter).to receive(:format_compile).with("NSMutableArray+ObjectiveSugar.m", "/Users/musalj/code/OSS/ObjectiveSugar/Classes/NSMutableArray+ObjectiveSugar.m", "foo")
      @parser.parse(SAMPLE_COMPILE)
    end

    it "parses compiling classes" do
      expect(@formatter).to receive(:format_compile).with("KWNull.m", "Classes/Core/KWNull.m", "foo")
      @parser.parse(SAMPLE_ANOTHER_COMPILE)
    end

    it "parses compiling Objective-C++ classes" do
      expect(@formatter).to receive(:format_compile).with("KWNull.mm", "Classes/Core/KWNull.mm", "foo")
      @parser.parse(SAMPLE_ANOTHER_COMPILE.sub('.m', '.mm'))
    end

    it 'parses compiling Swift source files' do
      expect(@formatter).to receive(:format_compile).with("Resource.swift", "/Users/paul/foo/bar/siesta/Source/Resource.swift", "foo")
      @parser.parse(SAMPLE_SWIFT_COMPILE)
    end

    it "parses compiling C and C++ files" do
      ['.c', '.cc', '.cpp', '.cxx'].each do |file_extension|
        expect(@formatter).to receive(:format_compile).with("KWNull" + file_extension, "Classes/Core/KWNull" + file_extension, "foo")
        @parser.parse(SAMPLE_ANOTHER_COMPILE.sub('.m', file_extension))
      end
    end

    it "parses compiling metal files" do
      expect(@formatter).to receive(:format_compile_metal).with("MYMetalFile.metal", "CocoaChip/metalSample/MYMetalFile.metal", "foo")
      @parser.parse(SAMPLE_COMPILE_METAL)
    end

    it "parses compiling XIBs" do
      expect(@formatter).to receive(:format_compile_xib).with("MainMenu.xib", "CocoaChip/en.lproj/MainMenu.xib", "foo")
      @parser.parse(SAMPLE_COMPILE_XIB)
    end

    it "parses compiling storyboards" do
      expect(@formatter).to receive(:format_compile_storyboard).with("Main.storyboard", "sample/Main.storyboard", "foo")
      @parser.parse(SAMPLE_COMPILE_STORYBOARD)
    end

    it "parses compiling asset catalogs" do
      expect(@formatter).to receive(:format_compile_asset_catalog).with("sample/Assets.xcassets", "foo")
      @parser.parse(SAMPLE_COMPILE_ASSET_CATALOG)
    end

    it 'parses CopyPlistFile' do
      expect(@formatter).to receive(:format_copy_plist_file).with(
    ...
    it "parses CopyStringsFile" do
      expect(@formatter).to receive(:format_copy_strings_file).with('InfoPlist.strings', "foo")
      @parser.parse(SAMPLE_COPYSTRINGS)
    end

    it "parses CpHeader" do
      expect(@formatter).to receive(:format_copy_header_file).with(
    ...
    it "parses CpResource" do
      expect(@formatter).to receive(:format_cpresource).with('ObjectiveSugar/Default-568h@2x.png', "foo")
      @parser.parse(SAMPLE_CPRESOURCE)
    end

    it "parses GenerateDSYMFile" do
      expect(@formatter).to receive(:format_generate_dsym).with('ObjectiveSugarTests.octest.dSYM', "foo")
      @parser.parse(SAMPLE_DSYM)
    end

    it "parses info.plist processing" do
      expect(@formatter).to receive(:format_process_info_plist).with('The Spacer-Info.plist', 'The Spacer/The Spacer-Info.plist', "foo")
      @parser.parse(SAMPLE_PROCESS_INFOPLIST)
    end

    it "parses Ld" do
      expect(@formatter).to receive(:format_linking).with('ObjectiveSugar', 'normal', 'i386', nil)
      @parser.parse(SAMPLE_LD)
    end

    it "parses Ld with relative path" do
      expect(@formatter).to receive(:format_linking).with('ObjectiveSugar', 'normal', 'i386', "foo")
      @parser.parse(SAMPLE_LD_RELATIVE)
    end

    it "parses MetalLink" do
      expect(@formatter).to receive(:format_linking_metal).with('default.metallib', "foo")
    ...
    it "parses Libtool" do
      expect(@formatter).to receive(:format_libtool).with('libPods-ObjectiveSugarTests-Kiwi.a', "foo")
      @parser.parse(SAMPLE_LIBTOOL)
    end

    it "parses uitest failing tests" do
      expect(@formatter).to receive(:format_failing_test).with(
    ...
    it "parses specta failing tests" do
      expect(@formatter).to receive(:format_failing_test).with("SKWelcomeViewControllerSpecSpec",
    ...
    it "parses old specta failing tests" do
      expect(@formatter).to receive(:format_failing_test).with("RACCommandSpec",
    ...
    it "parses address sanitizer runtime errors" do
      expect(@formatter).to receive(:format_runtime_error).with(
        "AddressSanitizer",
    ...
    it "parses ld bitcode errors" do
      expect(@formatter).to receive(:format_error).with(SAMPLE_BITCODE_LD.strip)
      @parser.parse(SAMPLE_BITCODE_LD)
    end

    it "parses passing ocunit tests" do
      expect(@formatter).to receive(:format_passing_test).with('RACCommandSpec',
    ...
    it "parses passing specta tests" do
      expect(@formatter).to receive(:format_passing_test).with('SKWelcomeActivationViewControllerSpecSpec',
    ...
    it "parses pending tests" do
      expect(@formatter).to receive(:format_pending_test).with('TAPIConversationSpec',
    ...
    it 'parses measuring tests' do
      expect(@formatter).to receive(:format_measuring_test).with(
    ...
    it "parses build success indicator" do
      expect(@formatter).to receive(:format_phase_success).with('BUILD')
      @parser.parse(SAMPLE_BUILD_SUCCEEDED)
    end

    it "parses clean success indicator" do
      expect(@formatter).to receive(:format_phase_success).with('CLEAN')
      @parser.parse(SAMPLE_CLEAN_SUCCEEDED)
    end

    it "parses PhaseScriptExecution" do
      expect(@formatter).to receive(:format_phase_script_execution).with('Check Pods Manifest.lock', "foo")
      @parser.parse(SAMPLE_RUN_SCRIPT)
    end

    it "parses RuleScriptExecution" do
      expect(@formatter).to receive(:format_script_rule_execution).with('File Built With Custom Script.jjj', 'My Folder/a/File Built With Custom Script.jjj', "foo")
      @parser.parse(SAMPLE_CUSTOM_SCRIPT_BUILD_RULE)
    end

    it "parses process PCH" do
      expect(@formatter).to receive(:format_process_pch).with("Pods-CocoaLumberjack-prefix.pch", "foo")
      @parser.parse(SAMPLE_PRECOMPILE)
    end

    it 'parses process PCH command' do
      compile_statement = SAMPLE_PRECOMPILE.lines.to_a.last
      expect(@formatter).to receive(:format_process_pch_command).with("/Users/musalj/code/OSS/ObjectiveRecord/Pods/Pods-CocoaLumberjack-prefix.pch", nil)
      @parser.parse(compile_statement)
    end

    it "parses preprocessing" do
      expect(@formatter).to receive(:format_preprocess).with("CocoaChip/CocoaChip-Info.plist", "foo")
      @parser.parse(SAMPLE_PREPROCESS)
    end

    it "parses PBXCp" do
      expect(@formatter).to receive(:format_pbxcp).with("build/Release/CocoaChipCore.framework", "foo")
      @parser.parse(SAMPLE_PBXCP)
    end

    it 'parses changing directories' do
      expect(@formatter).to receive(:format_shell_command).with('cd',
                                                            '/some/place/out\ there')
      @parser.parse('    cd /some/place/out\ there')
    end

    it 'parses any indented command' do
      expect(@formatter).to receive(:format_shell_command).with(
    ...
    it "parses Touch" do
      expect(@formatter).to receive(:format_touch).with(
    ...
    it "parses write file" do
      expect(@formatter).to receive(:format_write_file).with(
    ...
    it "parses write auxiliary files" do
      expect(@formatter).to receive(:format_write_auxiliary_files)
      @parser.parse(SAMPLE_WRITE_AUXILIARY_FILES)
    end

    it "parses create build directory" do
      expect(@formatter).to receive(:format_create_build_directory).with("build", "foo")
      @parser.parse(SAMPLE_CREATE_BUILD_DIRECTORY)
    end

    it "parses mkdir" do
      expect(@formatter).to receive(:format_mkdir).with("OnePasswordExtension.framework", "foo")
      @parser.parse(SAMPLE_MKDIR)
    end

    it "parses process product packaging" do
      expect(@formatter).to receive(:format_process_product_packaging).with(
        "Foo.entitlements",
        "MobileApp.app.xcent",
        "foo"
      )
    ...
    it "parses ditto" do
      expect(@formatter).to receive(:format_ditto).with(
    ...
    it "parses compile DTrace script" do
      expect(@formatter).to receive(:format_compile_dtrace_script).with(
    ...
    it "parses copy PNG file" do
      expect(@formatter).to receive(:format_copy_png_file).with(
    ...
    it "parses copy TIFF file" do
      expect(@formatter).to receive(:format_copy_tiff_file).with(
    ...
    it "parses link storyboasrds" do
      expect(@formatter).to receive(:format_link_storyboards).with("foo")
      @parser.parse(SAMPLE_LINK_STORYBOARDS)
    end

    it "parses note" do
      expect(@formatter).to receive(:format_note).with(
    ...
    it "parses write auxiliary file" do
      expect(@formatter).to receive(:format_write_auxiliary_file).with("Specta.hmap", "foo")
      @parser.parse(SAMPLE_WRITE_AUXILIARY_FILE)
    end

    it "parses processing" do
      expect(@formatter).to receive(:format_processing_file).with(
    ...
    it "parses signing identity" do
      expect(@formatter).to receive(:format_signing_identity).with("-")
      @parser.parse(SAMPLE_SIGNING_IDENTITY)
    end

    it "parses TiffUtil" do
      expect(@formatter).to receive(:format_tiffutil).with('eye_icon.tiff', "foo")
      @parser.parse(SAMPLE_TIFFUTIL)
    end

    it "parses undefined symbols" do
      expect(@formatter).to receive(:format_undefined_duplicate_symbols).with(
    ...
    it "parses ocunit test run finished" do
      expect(@formatter).to receive(:format_test_run_finished).with('ReactiveCocoaTests.octest(Tests)', '2013-12-10 07:03:03 +0000.')
      @parser.parse(SAMPLE_OCUNIT_TEST_RUN_COMPLETION)
    end

    it "parses ocunit test run passed" do
      expect(@formatter).to receive(:format_test_run_finished).with('Hazelnuts.xctest', '2014-09-24 23:09:20 +0000.')
      @parser.parse(SAMPLE_OCUNIT_PASSED_TEST_RUN_COMPLETION)
    end

    it "parses ocunit test run failed" do
      expect(@formatter).to receive(:format_test_run_finished).with('Macadamia.octest', '2014-09-24 23:09:20 +0000.')
      @parser.parse(SAMPLE_OCUNIT_FAILED_TEST_RUN_COMPLETION)
    end

    it "parses specta test run finished" do
      expect(@formatter).to receive(:format_test_run_finished).with('KIFTests.xctest', '2014-02-28 15:44:32 +0000.')
      @parser.parse(SAMPLE_SPECTA_TEST_RUN_COMPLETION)
    end

    it "parses ocunit test run started" do
      expect(@formatter).to receive(:format_test_run_started).with('ReactiveCocoaTests.octest(Tests)')
      @parser.parse(SAMPLE_OCUNIT_TEST_RUN_BEGINNING)
    end

    it "parses specta test run started" do
      expect(@formatter).to receive(:format_test_run_started).with('KIFTests.xctest')
      @parser.parse(SAMPLE_SPECTA_TEST_RUN_BEGINNING)
    end

    it "parses ocunit test suite started" do
      expect(@formatter).to receive(:format_test_suite_started).with('RACKVOWrapperSpec')
      @parser.parse(SAMPLE_OCUNIT_SUITE_BEGINNING)
    end

    it "parses specta test suite started" do
      expect(@formatter).to receive(:format_test_suite_started).with('All tests')
      @parser.parse(SAMPLE_SPECTA_SUITE_BEGINNING)
    end

    it "parses unknown strings" do
      expect(@formatter).to receive(:format_other).with(
    ...
    context "errors" do
      it "parses clang errors" do
        expect(@formatter).to receive(:format_error).with(SAMPLE_CLANG_ERROR)
        @parser.parse(SAMPLE_CLANG_ERROR)
      end

      it "parses cocoapods errors" do
        expect(@formatter).to receive(:format_error).with("error: The sandbox is not in sync with the Podfile.lock. Run 'pod install' or update your CocoaPods installation.")
        @parser.parse(SAMPLE_PODS_ERROR)
      end

      it "parses compiling errors" do
        expect(@formatter).to receive(:format_compile_error).with(
          "SampleTest.m",
      ...
      it 'parses file missing errors' do
        expect(@formatter).to receive(:format_file_missing_error).with(
      ...
      it 'parses fatal error: on the beginning of the line for corrupted AST files' do
        expect(@formatter).to receive(:format_error).with(
      ...
      it "doesn't print the same error over and over" do
        ...
        expect(@formatter).to_not receive(:format_compile_error)
        @parser.parse("hohohoooo")
      end

      it "parses provisioning profile doesn't support capability error" do
        expect(@formatter).to receive(:format_error)
    ...
    context "warnings" do
      it 'parses generic warnings' do
        expect(@formatter).to receive(:format_warning).with(
      ...
      it "parses compiling warnings" do
        expect(@formatter).to receive(:format_compile_warning).with(
          "AppDelegate.m",
      ...
      it "parses ld warnings" do
        expect(@formatter).to receive(:format_ld_warning).with("ld: embedded dylibs/frameworks only run on iOS 8 or later")
      ...
      it "parses will not be code signed warnings" do
        expect(@formatter).to receive(:format_will_not_be_code_signed).with(SAMPLE_WILL_NOT_BE_CODE_SIGNED)
    ...
    context "summary" do
      ...
      it "returns empty string if the suite is not done" do
        expect(@parser.parse(SAMPLE_EXECUTED_TESTS)).to eq("")
      end

      it "knows when the test suite is done for OCunit" do
        given_tests_are_done
        expect(@formatter).to receive(:format_test_summary)
        @parser.parse(SAMPLE_EXECUTED_TESTS)
      end
      ...
      it "doesn't print executed message twice for Kiwi tests" do
        expect(@formatter).to receive(:format_test_summary).once
      ...
      it "prints OCunit / XCTest summary twice if tests executed twice" do
        expect(@formatter).to receive(:format_test_summary).twice
        2.times {
